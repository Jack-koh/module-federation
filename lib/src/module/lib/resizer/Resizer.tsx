...
export const Resizer: React.FC<Props> = ({
  ...
  const Resize = (e: MouseEvent): void => {
    if (targetRef.current) {
      ...
      if (position === "right" || position === "left") {
        document.body.classList.add("col__resizing");
        ...
        document.body.classList.add("row__resizing");
  ...
  const resizerEl = React.createElement(
    "div",
    { className: `custom__resizer ${position}`, onMouseDown: resizeHandler },
    React.createElement(
      "div",
      { className: `resizer__grap` },
      React.createElement("div", { className: "hover-action" }),
    ),
  );
  ...
  const cloneEl = React.Children.map(CHILDREN, (child: JSX.Element): JSX.Element => {
    ...
    const properties = {
      style,
      ref: targetRef,
      className: classNames("custom__resize__wrapper", { [className]: className }),
    };

    return React.cloneElement(child, properties, CHILDREN.props.children, resizerEl);
  });
