...
const positionHandler = (args: {
  ...
  const { gap, arrow, keepalive, setToggle, tooltipRoot, arrowElement, target } = args;
  let { position } = args;
  ...
  const v_center = targetRect.top + targetRect.height / 2 - tooltipRoot.clientHeight / 2;
  ...
  const positionHandler = (standard: string, value: { direction: number; align: number }): void => {
  ...
  const toolTipRect: DOMRect = tooltipRoot.getBoundingClientRect();
  ...
  const arrowPositionHandler = (): void => {
    ...
    const getVerticalLeft = (): number =>
      targetRect.width > toolTipRect.width ? toolTipRect.left + toolTipRect.width / 2 - 5 : h_target_center - 5;
